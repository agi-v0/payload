...
function MorphingDialogProvider({
  ...
  const setIsOpen = useCallback(
    (value: React.SetStateAction<boolean>) => {
      _setIsOpen((prevIsOpen) => {
        const newIsOpen = typeof value === 'function' ? value(prevIsOpen) : value
        // Call the onOpenChange callback if it exists and the state has actually changed
        if (onOpenChangeRef.current && prevIsOpen !== newIsOpen) {
          onOpenChangeRef.current(newIsOpen)
        }
        return newIsOpen
      })
    },
    [_setIsOpen], // _setIsOpen from useState is stable
  )
      ...
      {isOpen && (
        <>
          <motion.div
            ...
            className="fixed inset-0 z-11 h-full w-full bg-neutral-950/10 backdrop-blur-xs dark:bg-white/30"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          />
